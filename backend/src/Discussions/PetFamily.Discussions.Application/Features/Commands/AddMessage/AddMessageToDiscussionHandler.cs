...
{
    public class AddMessageToDiscussionHandler : ICommandHandler<AddMessageToDiscussionCommand>
    {
        private readonly IValidator<AddMessageToDiscussionCommand> _validator;
        private readonly ILogger<AddMessageToDiscussionHandler> _logger;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (discussion.Value.Status.Value == DiscussionStatus.Status.Closed)
                return Error.AccessDenied(
                    "discussion.closed", 
                    "Can't add message to closed discussion").ToErrorsList();

            var text = Text.Create(command.Text).Value;
            var userId = UserId.Create(command.UserId);

            var addMessageResult = discussion.Value.AddMessage(text, userId);
            if (addMessageResult.IsFailure)
                return addMessageResult.Error.ToErrorsList();

            await _unitOfWork.SaveChanges();

            _logger.LogInformation(
                "Message has been uploaded to discussion with relation id = {id}", discussion.Value.RelationId);
